...
- `make_dmg_background.py` — regenerates the DMG window background art.
- `dmg-background.png` / `dmg-background@2x.png` — that art (committed).

### The styled DMG window

The installer opens to a styled window (paper background, a title, an arrow,
and a "drag to Applications" prompt) built with **create-dmg**
(`brew install create-dmg`). If create-dmg isn't installed — or it fails —
`sign_notarize.sh` falls back to a plain functional DMG, so a release never
breaks over cosmetics.

The background art is generated by `make_dmg_background.py` using the app's own
type — **Fraunces** (display) and **Inter** (body), the same variable fonts the
app registers in `src/views/tokens.py`. They're fetched on demand into
`.fonts-cache/` (gitignored); offline, it falls back to New York / SF. To tweak
the design (wording, weight, colours, layout) edit that script and re-run:

```bash
uv run --with pillow python packaging/macos/make_dmg_background.py
```

It writes a 1x (72 dpi) and a 2x (144 dpi) PNG; `sign_notarize.sh` combines
them into a Retina-crisp HiDPI TIFF with `tiffutil -cathidpicheck` at build
time. **Icon positions in the art and the `create-dmg` flags must stay in
sync** (window 660×400, app icon at 170,200, Applications at 490,200).
