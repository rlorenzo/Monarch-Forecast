...
- **`src/views/`**: Flet UI components: dashboard (tabbed: Overview/Transactions/Adjustments), about dialog (`about.py`, which carries the Monarch Money disclaimer — keep it in sync with README's Disclaimer section), chart (`flet-charts` `LineChart`), alerts, adjustments panel (ExpansionTile), transactions table, update banner.
...
- **Flet 0.86 API**: Charts use `LineChart` from `flet-charts` (`from flet_charts import LineChart`), NOT `MatplotlibChart` (which has canvas manager bugs). Use `ft.Border.all()` not `ft.border.all()`. Dialogs: `page.show_dialog()` / `page.pop_dialog()`, not `page.open()` / `page.close()`.
- **Imports**: `src` is the package root. Use `from src.data.preferences import ...` style. isort configured with `known-first-party = ["src"]`.
- **Line length**: 100 characters, enforced by ruff's `E501`. The formatter wraps most code, but it will not split a long string literal or a trailing comment — those you fix by hand (a `# nosemgrep:` directive can move to the line above; bandit's `# nosec` cannot).
...
- **Dialogs must be Escape-dismissable.** The global handler is in `src/main.py` on `page.on_keyboard_event`; don't swallow Escape inside a dialog. Primary action buttons should carry `autofocus=True` (or the dialog's first TextField should) so focus enters the modal. **Exception: a dialog whose primary action is destructive and irreversible gives focus to the safe action instead** — `autofocus` arms Return, and arming Return to destroy the user's data is the accident this rule exists to prevent. The requirement it serves is that focus enters the modal at all, which the safe action satisfies equally. `src/views/erase_data.py` is the one such dialog today: Cancel carries `autofocus=True`, Erase does not, and `tests/test_erase_data.py` pins that. Automated reviewers read the rule above as unconditional and flag this; it is deliberate.
...
- `asyncio_mode = "auto"` is set, so async test functions work without decorators.
...
- **A stale `build/macos/` silently blanks the desktop app.** `flet_desktop` resolves its client in priority order and step 1 is "any `.app` under `./build/macos/`" — the output of the last `flet build`. If that bundle was built against an older Flet than the installed server, `uv run monarch-forecast` opens a window that renders *nothing*: no error, no log line, just blank. This cost real debugging time after the 0.86 upgrade, when a bundle from 1.2.0 kept being picked. If the app launches blank, check what client it actually started (`ps -Ao command | grep -i flet`) before suspecting your code; a correct run points at `~/.flet/client/flet-desktop-full-<version>/Flet.app`. Rebuild or remove `build/macos/` to fix it.
- **`build.yml` only runs on `v*` tags, so nothing catches a broken release build until you tag.** Seven PRs of green CI sat on top of a release that could not build, because the Flutter pin in `build.yml` (`flutter-version:`) still matched Flet 0.85 after the 0.86 upgrade. `flet build` does not fail with a version error in that case — it offers to install the right SDK and blocks on a `[y/n]` prompt, which dies on a runner as `EOFError: EOF when reading a line`. **Bump the Flutter pin in the same PR as any flet/flet-cli bump**, and expect to verify a release build only by tagging.
