...
- **Stock Material defaults.** The current app uses `color_scheme_seed=Colors.BLUE` and otherwise inherits Material 3's defaults. It reads as "an unfinished Flutter app," not a considered product. A branded surface, opinionated chart, and deliberate type scale are required.
